...
@Component({
  ...
  styleUrls: ['./sixteen-segment.component.css']
})
export class SixteenSegmentComponent implements AfterViewInit {

  private _bitMask: number = 5376
  
  @Input() public width: number = 360
  @Input() public height: number = 500
  @Input() public randomNoise: boolean = false;
  @Input() public glowColor: string = '#51ff00'
  @Input() public glowBlur: number = 15
  @Input() public backgroundColor: string = '#000000'
  @Input() public colorOn: string = '#51ff00'
  @Input() public colorOff: string = 'rgba(255, 255, 255, 0.1)'
  ...
  drawCrossSegment (
    ...
    if (this.canvas && this.canvas.nativeElement) {
      ...
      if ( on ) {
        ctx.shadowBlur = this.glowBlur + ( this.randomNoise ? Math.floor( Math.random() * 5 ) : 0 );
      } else {
        ctx.shadowBlur = 0;
      } 
